...
*/import { NextResponse } from 'next/server';
import { API_CONFIG } from '@/config/api';
// ⚠️ Adjust this import path to locate your backend.ts file
import { buildUrl } from '../../../lib/backend';

export async function GET() {
  try {
    const tourTypesUrl = buildUrl(API_CONFIG.ENDPOINTS.DESTINATIONS.TOUR_TYPES);
    const countriesUrl = buildUrl(API_CONFIG.ENDPOINTS.DESTINATIONS.COUNTRIES);
    const statesUrl = buildUrl(API_CONFIG.ENDPOINTS.DESTINATIONS.STATES);
    const fixedDeparturesUrl = `${buildUrl(API_CONFIG.ENDPOINTS.FIXED_DEPARTURES.ALL)}?limit=100`;
    ...
    // --- START: Improved Error Checking ---
    const errors: string[] = [];
    if (!tourTypesResponse.ok) {
      errors.push(`Failed to fetch tour types (${tourTypesResponse.status}): ${tourTypesUrl}`);
    }
    if (!countriesResponse.ok) {
      errors.push(`Failed to fetch countries (${countriesResponse.status}): ${countriesUrl}`);
    }
    if (!statesResponse.ok) {
      errors.push(`Failed to fetch states (${statesResponse.status}): ${statesUrl}`);
    }
    if (!fixedDeparturesResponse.ok) {
      errors.push(`Failed to fetch fixed departures (${fixedDeparturesResponse.status}): ${fixedDeparturesUrl}`);
    }

    if (errors.length > 0) {
      // If there are any errors, join them and throw
    ...
    const tourTypeData = await Promise.all(
      allTourTypes.map(async (tourType: string) => {
        // Fetch destinations
        const destinationsResponse = await fetch(
          `${buildUrl(API_CONFIG.ENDPOINTS.DESTINATIONS.ALL)}?tourType=${tourType}&limit=100`
        );
        
        // Fetch packages
        const packagesResponse = await fetch(
          `${buildUrl(API_CONFIG.ENDPOINTS.PACKAGES.ALL)}?tourType=${tourType}&limit=100`
        );
