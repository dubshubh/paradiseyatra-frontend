...
export function getImageUrl(imageUrl: string | null): string | null {
  ...
  // If it's a relative uploads path, construct the full URL using env var
  if (imageUrl.startsWith('/uploads/')) {
    const backend = (process.env.NEXT_PUBLIC_BACKEND_URL || process.env.BACKEND_URL || '').trim();
    if (!backend) {
      // no backend configured — return the relative path (or null) to avoid throwing
      return imageUrl;
    }
    try {
      // new URL handles joining and avoids double-slash issues
